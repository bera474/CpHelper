...
public:
  ...
  LazySGT(int a_len, vector<ll> &a) : LazySGT(a_len) {
    arr = a; build(0, n - 1, 1);
  }
...
private:
  ...
  void apply(int index, int start, int end, Update &u) {
    if (start != end) {
      lazy[index] = 1; updates[index].combine(u, start, end);
    }
  ...
  void update(int start, int end, int index, int left, int right, Update &u) {
    ...
    if (start >= left && end <= right) {
      apply(index, start, end, u); return;
    }
  ...
  Node query(int start, int end, int index, int left, int right) {
    ...
    if (start >= left && end <= right) {
      pushdown(index, start, end); return tree[index];
    }
    pushdown(index, start, end);
    int mid = (start + end) / 2;
    Node l, r, ans;
...
struct Update1 {
  ll val;
  Update1() { val = 0; }
  Update1(ll val1) { val = val1; }
  void apply(Node1 &a, int start, int end) { a.val = val * (end - start + 1); }
  void combine(Update1 &new_update, int start, int end) { val = new_update.val;}
};